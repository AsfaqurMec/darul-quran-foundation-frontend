...
export default function DonationCard({ item }: { item: DonationItem }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    translateContent();
  }, [lang, item.title, item.description, item.locale]);
    ...
    <Link href={href as any} className="block rounded-2xl border border-gray-200 overflow-hidden bg-white shadow-sm hover:shadow-md transition-all duration-300 h-full flex flex-col group">
      ...
      <div className="p-5 md:p-6 flex-1 flex flex-col">
        ...
        <div className="mt-auto">
          <span className="inline-flex w-full justify-center items-center gap-2 rounded-xl bg-emerald-700 text-white hover:bg-emerald-800 font-extrabold px-4 py-3 transition-colors">
            <span>{t('donate')}</span>
          </span>
        </div>
      </div>
